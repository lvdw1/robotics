...
class OptiPlannerSwingUp(OptiPlannerBase):
    def __init__(self, urdf_model_path, forward_dynamics_casadi_path, parameters_model):
        ...
        self.x_start = np.array([0.0, 0.0, 0.0, 0.0])
        # Goal condition: [q0, q1, dq0, dq1]
        self.x_goal = np.array([np.pi, np.pi, 0.0, 0.0])
        self.x_min = [ -parameters_model["max_angle_joint0"], -parameters_model["max_angle_joint1"] ]
        self.x_max = [  parameters_model["max_angle_joint0"],
                       parameters_model["max_angle_joint1"] ]
        self.v_max = [ parameters_model["max_velocity_joint0"],
                       parameters_model["max_velocity_joint1"] ]
        self.u_max = 1
    ...
    def setup(self):
        ...
        ocp.subject_to(z[2] >= -self.v_max[0])
        ocp.subject_to(z[2] <= self.v_max[0])
        ocp.subject_to(z[3] >= -self.v_max[1])
        ocp.subject_to(z[3] <= self.v_max[1])
